...
class Danmaku:
    ...
    def to_dict(self):
        send_time = datetime.fromtimestamp(self.send_time).strftime('%Y-%m-%d %H:%M:%S')
        hours, remainder = divmod(self.dm_time, 3600)
        minutes, seconds = divmod(remainder, 60)
        dm_time = "{:02}:{:02}:{:02}".format(int(hours), int(minutes), int(seconds))
        return {
            'text': self.text,
            'dm_time': dm_time,
            'send_time': send_time,
